...
- **Eager-load `slugs` when rendering lists.** `Page::query()->with('slugs')->paginate()` makes every link, hreflang set and `<head>` on the page free of per-row queries. Writes always re-read the current row, and `slugHistory()` still queries — history lives in the non-current rows.
