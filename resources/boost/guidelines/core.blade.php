...
- **Gated page that should still pass link equity? Override `polyslugRobotsDirective()`.** A model `polyslugIsRoutable()` refuses gets `robots: none`, and `none` means `noindex, nofollow` — stronger than the gate, which is about indexability alone and says nothing about the links on the page. Return `['noindex', 'follow']` (or the string `'noindex, follow'`) for a draft or gated preview. The answer must still contain `noindex` or `none`: a permissive one would undo the gate, and an EMPTY one is worse, because `laravel/head` then writes no robots tag at all and the page is indexable by default. Both throw `MisconfiguredPolyslug`. The method is on the trait, not the contract, so saying nothing changes nothing.
